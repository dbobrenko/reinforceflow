...
class _GlobalDQNLearner(BaseDQNAgent):
    def __init__(self,
                 env,
                 net_fn,
                 log_dir,
                 name='GlobalAgent'):
        super(_GlobalDQNLearner, self).__init__(env=env, net_fn=net_fn, name=name)
    ...
    def train_on_batch(self, obs, actions, rewards, summarize=False):
        raise NotImplementedError('For training Async DQN, use AsyncDQNTrainer.')
    ...
    def close(self):
        super(_GlobalDQNLearner, self).close()
        if self.writer:
            self.writer.close()
...
class _ThreadDQNLearner(BaseDQNAgent, Thread):
    ...
    def build_train_graph(self, optimizer, learning_rate, optimizer_args=None,
        ...
        super(_ThreadDQNLearner, self).build_train_graph(optimizer, learning_rate,
                                                         optimizer_args, decay,
                                                         decay_args, gradient_clip, saver_keep)
    ...
    def run(self):
        ...
        while not self.global_agent.request_stop:
            ...
            summary_str = self._train_on_batch(np.vstack(batch_obs), batch_actions, batch_rewards,
                                               summarize)
            if summarize:
                prev_step = self.global_agent.obs_counter
                train_r = ep_reward.reset()
                train_q = ep_q.reset()
                logger.info("%s on-policy eval.: Average R: %.2f. "
                            "Average maxQ: %.2f. Step: %d. Ep: %d"
                            % (self._scope, train_r, train_q, prev_step, episode))
                if summary_str:
                    logs = [tf.Summary.Value(tag=self._scope + 'train_r', simple_value=train_r),
                            ...
                            tf.Summary.Value(tag=self._scope + 'episodes', simple_value=episode)
                            ]
...
class AsyncDQNTrainer(object):
    ...
    def train(self,
        ...
        global_agent = _GlobalDQNLearner(env=make_env(self.env),
                                         net_fn=self.net_fn,
                                         log_dir=log_dir,
                                         name='GlobalAgent')
        global_agent.build_train_graph(optimizer, learning_rate, optimizer_args=optimizer_args,
        ...
        if log_dir and tf.train.latest_checkpoint(log_dir) is not None:
            global_agent.load_weights(log_dir)
        ...
        last_log_step = global_agent.obs_counter
        ...
        while has_live_threads() and global_agent.obs_counter < steps:
            try:
                if render:
                    for env in envs:
                        env.render()
                step = global_agent.obs_counter
