...
# Predefined custom _action space in given games
...
class GymWrapper:
    ...
    def __init__(self, env, actrep=4, memlen=4, w=84, h=84, random_start=30):
        ...
        if hasattr(self.env.action_space, "n"):
            ...
            raise ValueError("Environment %s: Unable to get _action space size." % self.env.spec.id)
    ...
    def set_custom_actions(self, action_space):
        """Sets custom _action space for current environment.
    ...
    def reset_random(self):
        ...
        for i in range(skip):
            ...
            if term:
                print('WARNING! Random _start frame skip have reached terminal _obs. Resetting.')
                s = self.env.reset()
        return self.preprocess(s, new_game=True)

    def step(self, action_index, test=False):
        """Executes _action and repeat it on the next X frames
        :param action_index: _action index (0-based)
        :type action_index: int
        :return 4 elements tuple:
               preprocessed and stacked screen,
               accumulated _reward over skipped frames
               is terminal,
               info
        :rtype: tuple"""
